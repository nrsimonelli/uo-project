// AUTO-GENERATED FILE. DO NOT EDIT.
// Source: shield.json

export const EquipmentShield = [
  {
    "id": "angelHuntersBuckler",
    "name": "Angel Hunter's Buckler",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": "aerialGuard",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "azureCrestShield",
    "name": "Azure Crest Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25,
      "PP": 1
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "baroqueShield",
    "name": "Baroque Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 4,
      "GRD": 18,
      "GuardEff": 25,
      "ACC": 5
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "battlersShield",
    "name": "Battler's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 2,
      "GRD": 5,
      "GuardEff": 25,
      "PATK": 2
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "beastHuntersBuckler",
    "name": "Beast Hunter's Buckler",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": "bestralGuard",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "blackIronShield",
    "name": "Black Iron Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 22,
      "GuardEff": 25,
      "CRT": 4
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "blessedRoundshield",
    "name": "Blessed Roundshield",
    "type": "Shield",
    "stats": {
      "PDEF": 2,
      "GRD": 20,
      "GuardEff": 25,
      "MaxHP": 5,
      "OnActiveHealPercent": 10
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "blueRoseShield",
    "name": "Blue Rose Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 20,
      "GuardEff": 25,
      "PP": 1
    },
    "skillId": "royalGuard",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "bronzeShield",
    "name": "Bronze Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 3,
      "GRD": 15,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "carnatShield",
    "name": "Carnat Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 24,
      "GuardEff": 25,
      "INIT": 1
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "cavalryHuntersBuckler",
    "name": "Cavalry Hunter's Buckler",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": "cavalryGuard",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "chivalricShield",
    "name": "Chivalric Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 25,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [
      "Stun",
      "GuardSeal"
    ],
    "classRestrictions": [
      "Dark Knight",
      "Doom Knight",
      "Great Knight",
      "High Lord",
      "Knight",
      "Paladin",
      "Radiant Knight",
      "Sainted Knight"
    ]
  },
  {
    "id": "dragonboneShield",
    "name": "Dragonbone Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 15,
      "GuardEff": 25,
      "PP": 1,
      "INIT": -10
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "goldenRamShield",
    "name": "Golden Ram Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25,
      "MaxHP": 10
    },
    "skillId": null,
    "nullifications": [
      "Stun"
    ],
    "classRestrictions": []
  },
  {
    "id": "greatwoodShield",
    "name": "Greatwood Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 4,
      "GRD": 15,
      "GuardEff": 25,
      "MDEF": 2
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "guardsShield",
    "name": "Guard's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 4,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": "quickGuard",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "heavenwingShield",
    "name": "Heavenwing Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25,
      "ACC": 10,
      "EVA": 10
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "hoarfrostShield",
    "name": "Hoarfrost Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [
      "Freeze"
    ],
    "classRestrictions": []
  },
  {
    "id": "holyKnightsShield",
    "name": "Holy Knight's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 7,
      "GRD": 30,
      "GuardEff": 25,
      "OnActiveHealPercent": 10
    },
    "skillId": "holyGuard",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "holyUnicornShield",
    "name": "Holy Unicorn Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 8,
      "GRD": 30,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [
      "Debuff"
    ],
    "classRestrictions": []
  },
  {
    "id": "huntersBuckler",
    "name": "Hunter's Buckler",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 25,
      "GuardEff": 25,
      "PursuitPotency": 20,
      "CRT": 5
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "icefallShield",
    "name": "Icefall Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 7,
      "GRD": [
        20,
        -20
      ],
      "GuardEff": 25,
      "PP": 1
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "ironShield",
    "name": "Iron Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "ironcladBuckler",
    "name": "Ironclad Buckler",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": "wall",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "kaikaiasShield",
    "name": "Kaikaias' Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 8,
      "GRD": 30,
      "GuardEff": 25,
      "INIT": 10
    },
    "skillId": "hastenedCover",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "luminousShield",
    "name": "Luminous Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 25,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [
      "Poison",
      "Blind"
    ],
    "classRestrictions": []
  },
  {
    "id": "manalithBuckler",
    "name": "Manalith Buckler",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": "magicWall",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "mercenarysShield",
    "name": "Mercenary's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 18,
      "GuardEff": 25,
      "CRT": 5
    },
    "skillId": "vengefulGuard",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "moonlightShield",
    "name": "Moonlight Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 25,
      "GuardEff": 25
    },
    "skillId": "nocturnalGuard",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "namelessGuardsShield",
    "name": "Nameless Guard's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 25,
      "GuardEff": 25
    },
    "skillId": "radiantCover",
    "nullifications": [
      "Blind"
    ],
    "classRestrictions": []
  },
  {
    "id": "parryingShield",
    "name": "Parrying Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 2,
      "GRD": 10,
      "GuardEff": 25,
      "CounterAttackPotency": 50
    },
    "skillId": "parry",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "phantomKnightsShield",
    "name": "Phantom Knight's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 0,
      "GRD": 0,
      "GuardEff": 25,
      "MDEF": 20
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "recruitsShield",
    "name": "Recruit's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 3,
      "GRD": 16,
      "GuardEff": 25,
      "MaxHP": 3,
      "ExpGainPercent": 10
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "scarletCrestShield",
    "name": "Scarlet Crest Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 25,
      "GuardEff": 25,
      "AP": 1
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "searingShield",
    "name": "Searing Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [
      "Burn"
    ],
    "classRestrictions": []
  },
  {
    "id": "spellsteelShield",
    "name": "Spellsteel Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 20,
      "GuardEff": 25,
      "MDEF": 5
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "squiresShield",
    "name": "Squire's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": "quickCover",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "steelShield",
    "name": "Steel Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 7,
      "GRD": 25,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "templarsShield",
    "name": "Templar's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25,
      "hpRecoveryPercent": 20
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "thornShield",
    "name": "Thorn Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25,
      "PP": 1,
      "MaxHP": -15
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "unfetteredShield",
    "name": "Unfettered Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 25,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [
      "GuardSeal",
      "PassiveSeal"
    ],
    "classRestrictions": []
  },
  {
    "id": "unyieldingShield",
    "name": "Unyielding Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 7,
      "GRD": 25,
      "GuardEff": 25
    },
    "skillId": "enduringGuard",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "viperShield",
    "name": "Viper Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 5,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [
      "Poison"
    ],
    "classRestrictions": []
  },
  {
    "id": "vorpalShield",
    "name": "Vorpal Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 7,
      "GRD": 25,
      "GuardEff": 25,
      "MaxHP": 5,
      "INIT": 3
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "watchmansBuckler",
    "name": "Watchman's Buckler",
    "type": "Shield",
    "stats": {
      "PDEF": 4,
      "GRD": 20,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [
      "Blind"
    ],
    "classRestrictions": []
  },
  {
    "id": "whiteKnightsShield",
    "name": "White Knight's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 4,
      "GRD": 15,
      "GuardEff": 25,
      "MaxHP": 15
    },
    "skillId": "lifeshare",
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "whiteRidersShield",
    "name": "White Rider's Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 7,
      "GRD": 25,
      "GuardEff": 25
    },
    "skillId": null,
    "nullifications": [
      "Stun",
      "GuardSeal"
    ],
    "classRestrictions": []
  },
  {
    "id": "wingcrestShield",
    "name": "Wingcrest Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 7,
      "GRD": 25,
      "GuardEff": 25,
      "PP": 1,
      "CRT": -20
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "zenoiranShield",
    "name": "Zenoiran Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 3,
      "GRD": 15,
      "GuardEff": 25,
      "MaxHP": 10
    },
    "skillId": null,
    "nullifications": [],
    "classRestrictions": []
  },
  {
    "id": "zenoiranKnightShield",
    "name": "Zenoiran Knight Shield",
    "type": "Shield",
    "stats": {
      "PDEF": 6,
      "GRD": 25,
      "GuardEff": 25,
      "MaxHP": 15
    },
    "skillId": "enduringGuard",
    "nullifications": [],
    "classRestrictions": []
  }
] as const;

export type EquipmentShieldId = (typeof EquipmentShield)[number]["id"];

export type EquipmentShieldMap = {
  [K in EquipmentShieldId]: Extract<(typeof EquipmentShield)[number], { id: K }>;
};

export const EquipmentShieldMap: EquipmentShieldMap = Object.fromEntries(
  EquipmentShield.map(item => [item.id, item])
) as EquipmentShieldMap;
